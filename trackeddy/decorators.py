...
def filter_data(func):
    """
    filter_data Wrapper to decide filtering to apply to the date,
                the options are 'space', 'time', and 'both'
...
def check_return_eddies(func):
    @wraps(func)
    def wrapper(self, *args, **kwargs):

        eddy_info, discarded, eddy = func(self, *args, **kwargs)

        if isinstance(eddy_info, pd.DataFrame):
            eddy.exit()
            eddy_info = eddy_info.set_index(["identifier", "index"])
        else:
            eddy_info = pd.DataFrame({"": []})

        if not isinstance(discarded, pd.DataFrame):
            discarded = []
