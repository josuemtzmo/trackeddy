...
class TrackEddy(object):
    ...
    def filter_data_spatially(self):
        ...
        self.DataArray = self.DataArray - convolution.convolve(self.DataArray , kernel = ker)
...
def find_centre_and_max(contour_yx_deg,data_inside_contour):
    ...
    data_max = filters.maximum_filter(data_inside_contour, 3)
    ...
    *************** minimize_surface_fitting *******************
    ...
    while error > etol:    
        ...
        gaussian_bounds = (range_bounds(initial_guess[0],0.01), # Amplitud bounds
                        ...
                        [ii if ii < np.pi/2.1 else np.sign(ii)*np.pi/4 for ii in range_bounds(initial_guess[3],a_b_percent)], # a bounds
                        [ii if ii < np.pi/2.1 else np.sign(ii)*np.pi/4 for ii in range_bounds(initial_guess[4],a_b_percent)], # b bounds
