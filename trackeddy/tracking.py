...
class TrackEddy():
    ...
    def _scan_eddy_single_level(self,level,method='',fit_gaussian=True,geo=True):
        ...
        for n_contour in range(0,number_close_contours):
            ...
            # data_inside_contour = insideness_contour(data4gauss,centertop, level, self.mask_contour_opt)
            ...
            initial_guess = [ eddy_c_location[2], eddy_c_location[0], eddy_c_location[1],\
                              ellipse_dict['a'],ellipse_dict['b'],ellipse_dict['theta'] ]

            gausssianfitp  = minimize_surface_fitting(data4gauss.values,fixvalues,initial_guess=initial_guess)
...
def find_centre_and_max(contour_yx_deg,data_inside_contour):
    """
    
    """
    centre_contour = np.mean(contour_yx_deg,axis=0)

    data_max = filters.maximum_filter(data_inside_contour, 5)
    maxima_indx = np.where(data_inside_contour == data_max)

    maxima_loc = np.array([ data_inside_contour.Y.isel(Y=maxima_indx[0]).values, 
    ...
    if len(np.shape(maxima_indx)) == 2:
        dist = np.sqrt(np.sum(centre_contour[:,np.newaxis] - maxima_loc))
        index = np.argmin(dist)
        data_max = data_inside_contour.isel({'X':maxima_indx[1][index],'Y':maxima_indx[0][index]})
    else:
        data_max = data_inside_contour.max()

    return np.array([ *centre_contour, data_max.values])
...
def contour_ji_2_yx(contour_ji,coords_range,shape,geo=True):
    ...
    """
    return ((contour_ji/(np.array(shape)-1))*coords_range['range']) + coords_range['mins']
    ...
    *************** minimize_surface_fitting *******************
    ...
    while error > etol:    
        ...
        gaussian_bounds = (range_bounds(initial_guess[0],0.01), # Amplitud bounds
                        ...
                        range_bounds(initial_guess[3],a_b_percent), # a bounds
                        range_bounds(initial_guess[4],a_b_percent), # b bounds
...
def find_eddy_max_or_cmass(dataarray,contour,level,halo=3,eddy_center_method="maximum"):
    """
    """
    # TODO instead of using directly the coordinates, use the dimension 
    # to support no regular grids
    

    if eddy_center_method == 'maximum':
        eddy_c_location=contourmaxvalue(dataarray,dataarray.X.values,\
                                dataarray.Y.values,level)

        eddy_c_location[3]=eddy_c_location[3]
        eddy_c_location[4]=eddy_c_location[4]

    elif eddy_center_method == 'masscenter':
        eddy_c_location = centroidvalue(data_in_contour,dataarray.X.values,\
                            dataarray.Y.values,level)
